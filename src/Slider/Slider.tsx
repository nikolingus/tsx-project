...
const Slider: React.FC = () => {
  ...
  useEffect(() => {
    ...
    function updateSlider(): void {
      // Проверяем slides и первый элемент
  ...
  const slides: ISlideData[] = React.useMemo(
    () => [
      {
        id: 1,
        title: "Тяньмэнь",
        text: "Гора, расположенная в Китае, на северо-западе провинции Хунань, в Национальном парке горы Тяньмэнь. На гору можно подняться по канатной дороге, она часто упоминается в туристических гидах как «самая длинная высокогорная канатная дорога в мире» - она состоит из 98 вагонов, её общая длина 7455 м перепад высот составляет 1279 метров, а самый крутой угол подъёма 37°.",
      },
      {
        id: 2,
        title: "Река Лицзян",
        text: "Во II веке до н. э. по приказу Цинь Шихуанди эти реки соединил канал Линцюй, что позволило плавать из Янцзы в Сицзян (и далее в дельту Жемчужной реки) на лодках. В 1982 году Государственный совет Китая признал живописную зону реки Лицзян местом, имеющим живописное и историческое значение.",
      },
      {
        id: 3,
        title: "Чунцин",
        text: "Высокотехнологичный мегаполис и самый большой город в мире, его площадь составляет 82 400 км².",
      },
    ],
    []
  );
  ...
  return (
    <section className="slider" ref={sliderRef}>
      <div className="slider__container" ref={containerRef}>
        {slides.map((slide) => (
          <div
            className={`slider__slide slider__slide--${slide.id}`}
            key={slide.id}
          >
